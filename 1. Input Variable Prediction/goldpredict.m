...
clear
clc
load goldnet.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(GOLDPRICE); % the length of time series
...
for i=1:nint
goldintrare(i) = {GOLDPRICE(start-nint+i)};
end
...
plot1 = plot([cell2mat(goldrez(1:pp))' GOLDPRICE(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(GOLDPRICE(start+1:start+pp)-cell2mat(goldrez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(GOLDPRICE(start+1:start+i)-cell2mat(goldrez(1:i))');
