...
clear
clc
load usdcadask.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(USDCADASK); % the length of time series
...
for i=1:nint
USDCADASKintrare(i) = {USDCADASK(start-nint+i)};
end
% the expected outputs will be time_series(start+1:start+p) p - steps
% time_series(3610:3710) 100

for j = 1:pp
%rez(j) = crudeoilnet(intrare(nint-1+j),intrare(j:nint-1+j),layerStates); % calculeaza raspunsul retelei
USDCADASKrez(j) = sim(usdcadasknet,USDCADASKintrare(nint-1+j),USDCADASKintrare(j:nint-1+j));
USDCADASKintrare(nint+j)= USDCADASKrez(j); % add the output to input signal 
end
...
plot1 = plot([cell2mat(USDCADASKrez(1:pp))' USDCADASK(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(USDCADASK(start+1:start+pp)-cell2mat(USDCADASKrez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(USDCADASK(start+1:start+i)-cell2mat(USDCADASKrez(1:i))');
