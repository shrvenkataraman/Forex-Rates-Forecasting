...
clear
clc
load usdaudask.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(USDAUDASK); % the length of time series
...
for i=1:nint
usdaudaskintrare(i) = {USDAUDASK(start-nint+i)};
end
...
usdaudaskrez(j) = sim(usdaudasknet,usdaudaskintrare(nint-1+j),usdaudaskintrare(j:nint-1+j));
usdaudaskintrare(nint+j)= usdaudaskrez(j); % add the output to input signal 
end
...
plot1 = plot([cell2mat(usdaudaskrez(1:pp))' USDAUDASK(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(USDAUDASK(start+1:start+pp)-cell2mat(usdaudaskrez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(USDAUDASK(start+1:start+i)-cell2mat(usdaudaskrez(1:i))');
