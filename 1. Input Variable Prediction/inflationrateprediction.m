...
clear
clc
load inflationratenet.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(inflationrate); % the length of time series
...
for i=1:nint
inflationrateintrare(i) = {inflationrate(start-nint+i)};
end
% the expected outputs will be time_series(start+1:start+p) p - steps
% time_series(3610:3710) 100

for j = 1:pp
%rez(j) = inflationratenet(intrare(nint-1+j),intrare(j:nint-1+j),layerStates); % calculeaza raspunsul retelei
inflationraterez(j) = sim(inflationnet,inflationrateintrare(nint-1+j),inflationrateintrare(j:nint-1+j));
inflationrateintrare(nint+j)= inflationraterez(j); % add the output to input signal 
end
...
plot1 = plot([cell2mat(inflationraterez(1:pp))' inflationrate(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(inflationrate(start+1:start+pp)-cell2mat(inflationraterez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(inflationrate(start+1:start+i)-cell2mat(inflationraterez(1:i))');
