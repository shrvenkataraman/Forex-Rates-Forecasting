...
clear
clc
load usdgbpask.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(USDGBPASK); % the length of time series
...
for i=1:nint
usdgbpaskintrare(i) = {USDGBPASK(start-nint+i)};
end
...
usdgbpaskrez(j) = sim(usdgbpnet,usdgbpaskintrare(nint-1+j),usdgbpaskintrare(j:nint-1+j));
usdgbpaskintrare(nint+j)= usdgbpaskrez(j); % add the output to input signal 
end
...
plot1 = plot([cell2mat(usdgbpaskrez(1:pp))' USDGBPASK(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(USDGBPASK(start+1:start+pp)-cell2mat(usdgbpaskrez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(USDGBPASK(start+1:start+i)-cell2mat(usdgbpaskrez(1:i))');
