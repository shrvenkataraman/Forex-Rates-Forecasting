...
clear
clc
load crudeoilnet.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(CRUDEOILPRICE); % the length of time series
...
for i=1:nint
crudeoilintrare(i) = {CRUDEOILPRICE(start-nint+i)};
end
...
plot1 = plot([cell2mat(crudeoilrez(1:pp))' CRUDEOILPRICE(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(CRUDEOILPRICE(start+1:start+pp)-cell2mat(crudeoilrez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(CRUDEOILPRICE(start+1:start+i)-cell2mat(crudeoilrez(1:i))');
