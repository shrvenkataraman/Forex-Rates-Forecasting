...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(cpi); % the length of time series
...
plot1 = plot([cell2mat(cpirez(1:pp))' cpi(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(cpi(start+1:start+pp)-cell2mat(cpirez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(cpi(start+1:start+i)-cell2mat(cpirez(1:i))');
