...
clear
clc
load usdchfask.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(USDCHFASK); % the length of time series
...
for i=1:nint
USDCHFASKintrare(i) = {USDCHFASK(start-nint+i)};
end
...
USDCHFASKrez(j) = sim(usdchfnet,USDCHFASKintrare(nint-1+j),USDCHFASKintrare(j:nint-1+j));
USDCHFASKintrare(nint+j)= USDCHFASKrez(j); % add the output to input signal 
end
...
plot1 = plot([cell2mat(USDCHFASKrez(1:pp))' USDCHFASK(start+1:start+pp)],'Parent',axes1);
...
hold(axes1,'all');
% Create plot
plot(USDCHFASK(start+1:start+pp)-cell2mat(USDCHFASKrez(1:pp))');
% Create xlabel
...
for i=1:pp
    ep(i)=mse(USDCHFASK(start+1:start+i)-cell2mat(USDCHFASKrez(1:i))');
