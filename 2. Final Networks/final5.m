...
clear
clc
load finalnet5.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(targets5); % the length of time series
...
inp(:,i)= [{input(:,start-nint+i)}; targets5(start-nint+i)];
...
for j = 1:pp
rez(j) = final51net(inp(:,nint-1+j),inp(:,j:nint-1+j),layerStates); 
...
for i=1:pp
    ...
    ep1(i)=mae(target1(start+1:start+i)-cell2mat(rez(1:i))');
    ep2(i)=sse(target1(start+1:start+i)-cell2mat(rez(1:i))');
