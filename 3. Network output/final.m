...
clear
clc
load finaltrainednet.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(target); % the length of time series
...
inp(:,i)= [{input(:,start-nint+i)}; target(start-nint+i)];
...
for j = 1:pp
rez(j) = finalnet(inp(:,nint-1+j),inp(:,j:nint-1+j),layerStates); 
