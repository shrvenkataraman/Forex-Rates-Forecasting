...
clear
clc
load finalnet2.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(targets2); % the length of time series
...
inp(:,i)= [{input(:,start-nint+i)}; targets2(start-nint+i)];
...
for j = 1:pp
rez(j) = final2net(inp(:,nint-1+j),inp(:,j:nint-1+j),layerStates); 
