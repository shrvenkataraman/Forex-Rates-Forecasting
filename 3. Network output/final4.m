...
clear
clc
load finalnet4.mat; % load a saved neural network
...
pp = 60; % number of predictions that we will do 
        ...
        % network
sfarsit = length(targets4); % the length of time series
...
inp(:,i)= [{input(:,start-nint+i)}; targets4(start-nint+i)];
...
for j = 1:pp
rez(j) = final4net(inp(:,nint-1+j),inp(:,j:nint-1+j),layerStates); 
